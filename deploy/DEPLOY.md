...
При первом запуске ожидается `сжато файлов: 127`, `исходно: 21.9 МБ`, gzip около
3.3–3.4 МБ, brotli около 2.3 МБ.

При повторных запусках большинство файлов будет пропущено: шаг пересжимает
только то, у чего архив старше исходника. Строка вида `сжато файлов: 3,
пропущено без изменений: 124` — это норма, а не сбой. Так сделано ради заливки:
`rsync` считает файл изменившимся по размеру и времени правки, поэтому
пересозданный архив с тем же содержимым он отправил бы целиком, и каждое
обновление тащило бы на сервер лишние мегабайты. Полное пересжатие при
необходимости возвращается удалением архивов:

```bash
find site \( -name '*.gz' -o -name '*.br' \) -delete
```
